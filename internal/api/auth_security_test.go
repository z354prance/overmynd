...
func TestAdminWriteRejectsCrossOrigin(t *testing.T) {
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"http://overmynd.local/api/v1/services",
		bytes.NewBufferString(`{"type":"radarr","name":"Radarr","enabled":true,"base_url":"http://10.0.0.10:7878","credential":"test-key"}`),
	)
...
func TestAuthSetupRejectsCrossOrigin(t *testing.T) {
	handler, _ := testRouterWithoutAdmin(t)

	req := httptest.NewRequest(
		http.MethodPost,
		"http://overmynd.local/api/v1/auth/setup",
	...
	handler.ServeHTTP(recorder, req)
	if recorder.Code != http.StatusForbidden {
		t.Fatalf("cross-origin setup = %d, want %d: %s", recorder.Code, http.StatusForbidden, recorder.Body.String())
	}
}
